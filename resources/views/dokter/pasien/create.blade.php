...
<div class="card border-pink-400 shadow-sm mx-auto" style="max-width:600px;">
    ...
    <div class="card-body">
        <form action="{{ route('dokter.pasien.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-2">
                ...
                <input type="date" name="tanggal_berobat" id="tanggal_berobat" class="form-control border-pink-300 focus:border-pink-500 focus:ring focus:ring-pink-200 py-1" value="{{ old('tanggal_berobat') ?? date('Y-m-d') }}" required>
            </div>

            <!-- Submit / Batal -->
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-pink bg-pink-500 hover:bg-pink-600 text-white flex-grow-1">Simpan</button>
                <a href="{{ route('dokter.pasien.index') }}" class="btn btn-pink bg-pink-300 hover:bg-pink-400 text-white flex-grow-1">Batal</a>
            </div>
        </form>
    </div>
</div>
